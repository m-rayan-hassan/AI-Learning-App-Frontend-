...
const VideoOverviewTab = ({ documentId }: { documentId: string }) => {
  ...
  const loadVideoUrl = async (isPollingCall = false) => {
    try {
      setError(null);
      const cached = localStorage.getItem(`videoUrl_${documentId}`);

      if (cached && !isGenerating && !videoUrl) {
        setVideoUrl(cached);
        setLoading(false);
      }

      const res = await aiServices.getVideoOverviewUrl(documentId);
      const dbVideoData = res;

      if (dbVideoData) {
        // Check for failed generation
        if (typeof dbVideoData === "object" && dbVideoData.generationStatus === "failed") {
          if (isPollingCall) {
          ...
          aiServices.deleteVideoOverview(dbVideoData._id).catch(() => {});
        ...
        const dbUrl =
          typeof dbVideoData === "string" ? dbVideoData : dbVideoData.secureUrl;
        const generating =
          typeof dbVideoData === "object" &&
          (dbVideoData.generationStatus === "pending");

        if (generating) {
          setIsGenerating(true);
          setLoading(false);
        } else if (dbUrl) {
          localStorage.setItem(`videoUrl_${documentId}`, dbUrl);
          setVideoUrl(dbUrl);
          setIsGenerating(false);
          setLoading(false);
        }
  ...
  const handleVideoGenerate = async () => {
    ...
    try {
      ...
      setIsGenerating(true);
      await aiServices.generateVideo(documentId);
      await loadVideoUrl();
      toast("Video is being generated!", {
        id: toastId,
        icon: "ℹ️",
        style: {
          background: "#eff6ff",
      ...
      toast.error(error.message || "An error occurred while generating the video.", { id: toastId });
      setIsGenerating(false);
    }
  };
            ...
            {(loading || isGenerating) && !videoUrl ? (
              ...
              >
                ...
                >
                  ...
                  {generationFailed ? "Retry" : "Generate Video"}
                </Button>
              </motion.div>
            ) : (
              <motion.div
              ...
              >
                <div className="relative group aspect-video rounded-2xl overflow-hidden shadow-2xl bg-black border border-border/50">
                  ...
                  <div className="absolute top-4 left-4 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="bg-black/60 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-widest border border-white/10"></span>
                  </div>
                </div>
              </motion.div>
            )}
