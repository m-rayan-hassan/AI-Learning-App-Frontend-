...
import { 
  MessageSquare, FileText, Mic, ArrowRight, 
  PlayCircle, Layers, LayoutDashboard, User, Bot, 
  BookOpen, LogOut, Podcast, 
  Play, Volume2, 
  Lightbulb, Upload, Brain, Trophy, ChevronLeft, ChevronRight,
  Video, Eye, Sparkles, MessageCircle, Plus, Minus,
  ArrowUpRight, Moon, Zap
} from "lucide-react";
...
const DashboardSidebar = () => (
  <div className="w-56 border-r border-border hidden md:flex flex-col bg-background p-4 gap-2">
    <div className="flex items-center gap-2 mb-6 px-2">
      ...
      <span className="font-bold text-foreground tracking-tight text-sm">Cognivio<span className="text-primary">AI</span></span>
    </div>
...
const SidebarItem = ({ active, label, icon }: { active?: boolean, label: string, icon: React.ReactNode }) => (
  <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm cursor-pointer transition-all font-medium ${active ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/25' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}>
    {icon}
    <span>{label}</span>
  </div>
);
...
const PreviewView = () => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full bg-card/30 p-8 flex justify-center overflow-y-auto">
    <div className="bg-card w-full max-w-3xl min-h-[600px] shadow-xl rounded-lg p-12 border border-border">
      <h1 className="text-2xl font-bold mb-6 text-foreground">Introduction to Software Engineering</h1>
      <div className="space-y-4">
        <div className="h-3.5 bg-muted w-full rounded" />
        <div className="h-3.5 bg-muted w-full rounded" />
        <div className="h-3.5 bg-muted w-5/6 rounded" />
        <div className="h-48 bg-muted/50 w-full rounded-lg border border-border my-8 flex items-center justify-center text-muted-foreground text-sm">Figure 1.2 — System Architecture</div>
        <div className="h-3.5 bg-muted w-full rounded" />
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex h-full">
    <div className="flex-1 p-6 overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-foreground font-semibold flex items-center gap-2 text-sm"><Sparkles size={16} className="text-primary"/> Document Summary</h3>
        <Button size="sm" variant="ghost" className="h-7 text-xs text-primary bg-primary/10 hover:bg-primary/20">Regenerate</Button>
      </div>
      <div className="space-y-5 text-sm">
        <div className="p-5 rounded-xl bg-muted/50 border border-border">
          <h4 className="text-primary font-medium mb-3 text-xs uppercase tracking-wider">Executive Summary</h4>
          <p className="leading-relaxed text-muted-foreground">Risk management is a continuous, iterative process designed to proactively identify, analyze, and mitigate uncertainties affecting software projects. By cycling through identification and strategic planning, managers minimize the impact of technical, human, and organizational disruptions.</p>
        </div>
        <div>
          <h4 className="text-foreground font-medium mb-3 text-xs uppercase tracking-wider">Key Concepts</h4>
          <ul className="space-y-2">
            <li className="flex gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors">
              <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-primary shrink-0" />
              <span className="text-muted-foreground"><strong className="text-foreground block mb-0.5">Iterative Risk Process</strong>Functions as a feedback loop: Identification, Analysis, Planning, and Monitoring.</span>
            </li>
            <li className="flex gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors">
              <div className="mt-1.5 w-1.5 h-1.5 rounded-full bg-primary shrink-0" />
              <span className="text-muted-foreground"><strong className="text-foreground block mb-0.5">Project vs Product Risks</strong>Project risks affect schedule/resources. Product risks affect quality or performance.</span>
            </li>
          </ul>
        </div>
      </div>
    ...
    <div className="flex-1 space-y-5 overflow-y-auto pr-2">
      ...
      <div className="flex gap-3">
        ...
        <div className="bg-card border border-border text-foreground p-4 rounded-2xl rounded-tl-none text-sm max-w-[90%] space-y-2">
          <p className="text-muted-foreground">Based on the document, here is the distinction:</p>
          <ul className="list-disc pl-4 space-y-1 text-muted-foreground">
            <li><strong className="text-foreground">Project Risks:</strong> Affect the project schedule or resources (e.g., loss of experienced staff).</li>
            <li><strong className="text-foreground">Business Risks:</strong> Affect the organization causing the software to be developed.</li>
          </ul>
        </div>
      </div>
    </div>
    <div className="mt-4 relative">
      <input type="text" placeholder="Ask a follow-up question..." className="w-full bg-muted/50 border border-border rounded-xl py-3 pl-4 pr-12 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20" />
      <div className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-primary rounded-lg text-primary-foreground"><ArrowRight size={14}/></div>
    </div>
  </motion.div>
);
...
const FlashcardView = () => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col items-center justify-center p-6">
    <div className="w-full max-w-2xl aspect-[16/9] bg-card border border-border rounded-2xl flex flex-col items-center justify-center p-12 text-center relative cursor-pointer hover:border-primary/30 transition-colors shadow-xl">
      <div className="absolute top-6 left-6 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-semibold tracking-wider">QUESTION</div>
      <h3 className="text-xl md:text-2xl text-foreground font-semibold leading-tight">What are the 4 stages of the Risk Management Process?</h3>
      <div className="absolute bottom-6 text-muted-foreground text-xs flex items-center gap-2">
        Click to reveal answer
      </div>
    </div>
    <div className="mt-8 flex items-center gap-6">
      <Button variant="outline" className="gap-2"><ChevronLeft size={16}/> Prev</Button>
      <span className="text-muted-foreground font-mono text-sm">1 / 12</span>
      <Button className="bg-primary text-primary-foreground hover:bg-primary/90 gap-2">Next <ChevronRight size={16}/></Button>
    </div>
  </motion.div>
);
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-8 max-w-3xl mx-auto h-full flex flex-col justify-center">
    ...
    <div className="space-y-3">
      {["Reduce project scope to lower requirements.", "Request an immediate extension.", "Investigate acquisition of a higher-performance database.", "Allocate more staff to optimize code."].map((opt, i) => (
        <div key={i} className={`p-4 rounded-xl border ${i === 2 ? 'border-primary/50 bg-primary/5' : 'border-border bg-card hover:bg-muted/50'} cursor-pointer transition-colors flex items-center gap-3 group`}>
          <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${i === 2 ? 'border-primary' : 'border-muted-foreground/30 group-hover:border-muted-foreground/60'}`}>
            {i === 2 && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
          </div>
          <span className={i === 2 ? 'text-foreground' : 'text-muted-foreground'}>{opt}</span>
        </div>
      ))}
    </div>
    <div className="mt-8 flex justify-end">
      <Button className="bg-primary hover:bg-primary/90 text-primary-foreground px-6">Next Question</Button>
    </div>
  </motion.div>
);
...
const VoiceOverviewView = () => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col p-6">
    <h3 className="text-foreground font-medium flex items-center gap-2 mb-2 text-sm"><Mic className="text-primary w-4 h-4" /> Voice Overview</h3>
    <p className="text-muted-foreground text-xs mb-8">AI-generated audio summary of your document.</p>
    <div className="bg-muted/50 border border-border rounded-xl p-4 mt-4">
      ...
      <div className="bg-card rounded-lg p-3 flex items-center gap-4 border border-border">
        ...
        <span className="text-xs text-muted-foreground font-mono">0:00 / 2:02</span>
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-6 h-full flex flex-col">
    <div className="flex justify-between items-center mb-6">
      <div>
        ...
        <p className="text-xs text-primary font-mono mt-1">Interactive Learning</p>
      </div>
      <div className="bg-card rounded-lg p-1 flex gap-1 border border-border">
        <div className="px-3 py-1.5 rounded-md text-xs text-muted-foreground cursor-pointer hover:text-foreground">Discuss</div>
    ...
    <div className="flex-1 grid grid-cols-2 gap-4 mb-4">
      <div className="bg-muted/30 border border-border rounded-2xl flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="mt-3 text-center">
          ...
          <div className="text-xs text-primary/80 mt-1 bg-primary/10 px-2 py-0.5 rounded-full inline-block border border-primary/20">Active</div>
        </div>
      </div>
      <div className="bg-muted/30 border border-border rounded-2xl flex flex-col items-center justify-center">
        ...
        <div className="mt-3 text-center">
          ...
          <div className="text-xs text-muted-foreground mt-1">Microphone Off</div>
        </div>
      </div>
    </div>
    <div className="h-16 bg-muted/30 border border-border rounded-2xl p-3 flex gap-4 items-center">
      ...
      <div className="flex-1 h-full flex items-center px-2">
        <div className="flex gap-[3px] items-end h-8 w-full opacity-40">
          {[40, 65, 30, 80, 50, 70, 35, 90, 45, 60, 75, 40, 85, 55, 70, 30, 65, 50, 80, 45, 60, 35, 75, 55, 40, 70, 50, 85, 45, 65].map((h, i) => (
  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col p-6">
    <div className="w-full aspect-video bg-foreground/5 rounded-xl border border-border flex items-center justify-center relative group cursor-pointer overflow-hidden">
      <div className="absolute inset-0 bg-gradient-to-t from-background/80 to-transparent flex items-end p-6 z-10">
        <div>
          <h3 className="text-foreground font-bold text-xl mb-1">Navigating Project Risk</h3>
          <p className="text-muted-foreground text-sm">Chapter 22 — Deep Dive</p>
        </div>
      </div>
    ...
    <div className="mt-6 flex gap-6">
      <div className="w-56 hidden lg:block space-y-2">
        ...
        <div className="text-sm p-2 rounded bg-primary/10 text-primary border border-primary/20 cursor-pointer">0:00 Intro to Risk</div>
        <div className="text-sm p-2 rounded hover:bg-muted text-muted-foreground cursor-pointer transition-colors">2:15 Risk Identification</div>
        <div className="text-sm p-2 rounded hover:bg-muted text-muted-foreground cursor-pointer transition-colors">5:40 Mitigation Strategies</div>
      </div>
      <div className="flex-1">
        <h4 className="text-foreground font-medium mb-3 text-sm">Transcript</h4>
        <p className="text-muted-foreground text-sm leading-relaxed">
          &ldquo;...so when we talk about <span className="text-foreground bg-primary/10 px-1 rounded">project risk</span>, we are really discussing the uncertainties that threaten the schedule. Now, contrast that with product risk...&rdquo;
        </p>
      </div>
    </div>
  </motion.div>
...
const ConceptView = () => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col p-6">
    <h3 className="text-foreground font-medium flex items-center gap-2 mb-2 text-sm"><Lightbulb className="text-primary w-4 h-4" /> Explain Concept</h3>
    <p className="text-muted-foreground text-xs mb-6">Ask the AI to explain specific topics found in the document.</p>
    <div className="relative mb-8">
      <input type="text" placeholder="E.g., What is the main conclusion?" className="w-full bg-muted/50 border border-border rounded-lg py-3 pl-4 pr-10 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50" readOnly />
      <div className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-primary rounded-md text-primary-foreground"><Lightbulb size={12} /></div>
    </div>
  ...
  return (
    <div className="border border-border rounded-xl overflow-hidden transition-colors hover:border-primary/20">
      ...
      >
        ...
        <div className={`shrink-0 w-7 h-7 rounded-lg flex items-center justify-center transition-colors ${isOpen ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
      ...
      <main className="flex-1">
          ...
          <div className="container px-4 mx-auto relative z-10">
            <div className="flex flex-col items-center text-center max-w-4xl mx-auto mb-16">
              ...
              >
                ...
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-400">into interactive mastery.</span>
              </motion.h1>
              ...
              >
                Upload any document and instantly generate AI-powered summaries, flashcards, quizzes, 
                voice explanations, and video overviews — all in one workspace.
              </motion.p>
              ...
              >
                <Link href="/register">
                  <Button size="lg" className="rounded-full px-8 h-12 shadow-lg shadow-primary/20 text-sm font-medium">
                    Get Started Free
                    <ArrowRight className="ml-2 w-4 h-4"/>
                  </Button>
                </Link>
                <Link href="#how-it-works">
                  <Button size="lg" variant="outline" className="rounded-full px-8 h-12 text-sm font-medium">
                    See How It Works
                  </Button>
                </Link>
              </motion.div>
              ...
              <div className="relative bg-background rounded-xl border border-border overflow-hidden font-sans flex flex-col h-[500px] md:h-[600px] lg:h-[700px] w-full ring-1 ring-border/50">
                ...
                <div className="border-b border-border flex items-center bg-background shrink-0 gap-2 w-full">
                  ...
                  <div className="hidden md:flex items-center gap-2 text-sm text-muted-foreground px-4 py-3 min-w-fit border-r border-border">
                    ...
                    <span className="font-medium text-foreground text-xs">SE_Lecture_Week4.pdf</span>
                  </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                  ...
                  <div className="flex-1 bg-card/30 relative overflow-hidden w-full">
                    <AnimatePresence mode="wait">
                      ...
                      {activeTab === 'Voice Overview' && <VoiceOverviewView key="voice-overview" />}
                      {activeTab === 'Concept' && <ConceptView key="concept" />}
        ...
        <section id="features" className="py-24 md:py-32 relative border-t border-border/40">
          <div className="container px-4 mx-auto">
            ...
            >
              <motion.div variants={fadeInUp} className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-border bg-muted/50 text-muted-foreground text-xs font-medium uppercase tracking-wider mb-4">
                Features
              </motion.div>
              <motion.h2 variants={fadeInUp} className="text-3xl md:text-5xl font-bold mb-5 tracking-tight">Everything you need to learn effectively</motion.h2>
            ...
            >
              ...
              >
                ...
                <div className="mt-6 flex items-end justify-center gap-[3px] h-12 w-full opacity-50">
                  {[40, 65, 30, 80, 50, 70, 35, 90, 45, 60, 75, 40, 85, 55, 70, 30, 65, 50, 80, 45].map((h, i) => (
                    <motion.div
                      key={i}
              ...
              >
                <div className="flex flex-wrap gap-2 mt-3">
                  <span className="text-[10px] px-2 py-1 rounded bg-muted border border-border text-muted-foreground">Audio</span>
                  <span className="text-[10px] px-2 py-1 rounded bg-muted border border-border text-muted-foreground">Podcast</span>
                  <span className="text-[10px] px-2 py-1 rounded bg-muted border border-border text-muted-foreground">Concept</span>
                </div>
              </BentoCard>
        ...
        {/* ================= HOW IT WORKS ================= */}
        <section id="how-it-works" className="py-24 md:py-32 bg-muted/20 relative border-t border-border/40">
          <div className="container px-4 mx-auto">
            <motion.div
              initial="hidden"
              whileInView="visible"
            ...
            >
              ...
              <motion.h2 variants={fadeInUp} className="text-3xl md:text-5xl font-bold mb-5 tracking-tight">Three steps to mastery</motion.h2>
              <motion.p variants={fadeInUp} className="text-muted-foreground text-lg max-w-xl mx-auto">Upload your material, let AI do the heavy lifting, and focus on what matters — learning.</motion.p>
            </motion.div>
        ...
        {/* ================= FAQ SECTION ================= */}
        <section id="faq" className="py-24 md:py-32 relative border-t border-border/40">
          <div className="container px-4 mx-auto">
            <motion.div
              initial="hidden"
              whileInView="visible"
            ...
            >
              <motion.div variants={fadeInUp} className="text-center mb-12">
                ...
                <h2 className="text-3xl md:text-4xl font-bold mb-5 tracking-tight">Frequently asked questions</h2>
                <p className="text-muted-foreground text-lg">Everything you need to know about Cognivio AI.</p>
              </motion.div>
        ...
        <section className="py-24 md:py-32 relative overflow-hidden border-t border-border/40">
          ...
          >
            <motion.h2 variants={fadeInUp} className="text-3xl md:text-5xl lg:text-6xl font-bold tracking-tight mb-6 text-foreground">
            ...
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row justify-center gap-4">
              <Link href="/register">
                <Button size="lg" className="h-13 px-10 rounded-full text-base shadow-xl shadow-primary/20 bg-primary hover:bg-primary/90 text-primary-foreground font-medium transition-all hover:shadow-2xl hover:shadow-primary/30">
                  Start Learning Free
                  <ArrowRight className="ml-2 w-4 h-4" />
                </Button>
              </Link>
              <Link href="/pricing">
                <Button size="lg" variant="outline" className="h-13 px-10 rounded-full text-base font-medium">
                  View Pricing
...
function StepCard({ number, title, desc, icon, gradient }: { number: string, title: string, desc: string, icon: React.ReactNode, gradient: string }) {
  return (
    ...
    >
      ...
      <div className="text-xs font-semibold text-primary mb-2 tracking-wider uppercase">Step {number}</div>
...
function BentoCard({ title, description, icon, colSpan = "", rowSpan = "", children, className = "" }: BentoCardProps) {
  return (
    ...
    >
      ...
      {children && (
        <div className="mt-3 flex-1 w-full relative">
          {children}
        </div>
      )}
    </motion.div>
